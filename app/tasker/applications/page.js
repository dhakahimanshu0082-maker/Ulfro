  ...
  return (
    <ProtectedRoute requiredRole="tasker">
      ...
      <div className="page-container"><div className="page-content">
        <div className="page-header"><h1>My Applications 📝</h1><p>Track all your task applications</p></div>
        {loading ? <div className="page-loading"><LoadingSpinner /></div> : apps.length === 0 ? (
          <div className="empty-state"><div className="empty-state-icon">📭</div><div className="empty-state-title">No applications yet</div><div className="empty-state-desc">Browse tasks and apply!</div><Link href="/tasker/browse/" className="btn-primary">Browse Tasks</Link></div>
        ) : (
                ...
                <Link key={app.id} href={`/tasker/task/?id=${app.task?.id}`} style={{ textDecoration: 'none' }}>
                  <div className="application-card">
                    <div className="application-card-header">
                      <div><div style={{ fontWeight: 600, marginBottom: '0.3rem' }}>{cat?.icon} {app.task?.title}</div><div style={{ fontSize: '0.82rem', color: 'var(--gray)' }}>📍 {app.task?.location || 'Delhi'} · Budget: ₹{app.task?.budget}</div></div>
                    ...
                    <div className="application-card-footer">
                      ...
                      <span className={`application-status-badge status-${app.status}`}>{app.status === 'accepted' ? '✅ Accepted' : app.status === 'rejected' ? '❌ Rejected' : app.status === 'pending' ? '⏳ Pending' : app.status}</span>
                    </div>
                  </div>
                </Link>
              );
