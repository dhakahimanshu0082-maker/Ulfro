...
export async function createTask(taskData) {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      client:profiles(id, full_name, avatar_url, rating)
    `)
    .single();
...
export async function getTask(taskId) {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      client:profiles(id, full_name, avatar_url, rating, phone, city),
      applications:task_applications(
        id, proposed_price, message, status, created_at,
        tasker:profiles(id, full_name, avatar_url, rating, phone)
      ),
  ...
  let query = supabase
    ...
    .select(`
      *,
      ...
      assignment:task_assignments(
        tasker:profiles(id, full_name, avatar_url, rating)
      )
    `)
  ...
  let query = supabase
    ...
    .select(`
      *,
      task:tasks(
        *,
        client:profiles(id, full_name, avatar_url, rating, phone)
      )
    `)
